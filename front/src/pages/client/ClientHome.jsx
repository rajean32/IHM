...
export default function ClientHome() {
  ...
  return (
    <div className="client-home">
        ...
        {events.map(event => (
          ...
          >
            ...
            <div className="event-card-body">
              <h3>{event.titre}</h3>
              <p className="event-date">{formatDate(event.dateEvenement)}</p>
              <p className="event-price">{getPriceRange(event)}</p>
      ...
      {!loading && events.length === 0 && <div className="empty">Aucun événement trouvé.</div>}

      {hasMore && !loading && (
        <div style={{ textAlign: 'center', margin: '1rem 0' }}>
          <button className="btn-secondary" onClick={handleLoadMore}>Charger plus</button>
        </div>
      )}
    </div>
  )
