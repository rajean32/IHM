...
export default function BookingFlow() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
        const [detailResp, availableResp] = await Promise.all([
  ...
  const totalPrice = selectedSeats.reduce((sum, s) => sum + (s.prix || 0), 0)
    ...
    <div className="booking-flow">
      ...
      {step === 1 && (
        <>
          ...
          <div className="summary-bar">
            <div>
              <span><strong>{selectedSeats.length}</strong> place(s) sélectionnée(s)</span>
              {selectedSeats.length > 0 && (
                <div style={{ fontSize: '0.85rem', color: '#666', marginTop: '4px' }}>
                  {selectedSeats.map(s => s.numeroPlace).join(', ')}
                </div>
              )}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <span className="total">{totalPrice.toFixed(2)} €</span>
              <button className="btn-primary" disabled={selectedSeats.length === 0} onClick={handleConfirm}>
      ...
      {step === 2 && (
        <div style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          ...
          <h4 style={{ marginBottom: '0.5rem' }}>Places sélectionnées</h4>
              ...
              <div key={seat.numeroPlace} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{
                    ...
                    background: getTypeIndicator(seat.typePlace),
                  }}></span>
                  <strong>{seat.numeroPlace}</strong>
                  <span style={{ color: '#666' }}>Rang {seat.rang || seat.range}</span>
                  <span style={{
                    fontSize: '0.8rem', padding: '1px 8px', borderRadius: '8px',
                    background: `${getTypeIndicator(seat.typePlace)}22`,
                    color: getTypeIndicator(seat.typePlace),
                  }}>{seat.typePlace}</span>
                </div>
