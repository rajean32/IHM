...
class NumberedSeatingTicketPage extends StatefulWidget {
  final String ticketCode;
  const NumberedSeatingTicketPage({super.key, required this.ticketCode});

  @override
  State<NumberedSeatingTicketPage> createState() => _NumberedSeatingTicketPageState();
}

class _NumberedSeatingTicketPageState extends State<NumberedSeatingTicketPage> {
  bool _loading = true;
  String? _error;
  Map<String, dynamic>? _data;
  List<Map<String, dynamic>>? _nearbySeats;
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(width: 8),
                Text(type ?? 'STANDARD', style: TextStyle(fontWeight: FontWeight.w700, color: color, fontSize: 14)),
              ]),
            ),
            const SizedBox(height: 20),
            _buildSeatDisplay(rang, place, color),
            const SizedBox(height: 20),
            ...
            _row(AppLocalizations.of(context)!.clientTicketRow, rang),
            _row(AppLocalizations.of(context)!.clientTicketSeat, place),
            _row(AppLocalizations.of(context)!.clientTicketType, type ?? 'STANDARD'),
  ...
  Widget _buildSeatDisplay(String rang, String place, Color color) {
    return Container(
      ...
      child: Column(children: [
        const Icon(Icons.event_seat, size: 48, color: AppColors.textMuted),
        const SizedBox(height: 12),
        Text(AppLocalizations.of(context)!.clientTicketRow, style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
        const SizedBox(height: 4),
        Text(rang, style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
        const SizedBox(height: 12),
        Container(height: 1, width: 40, color: AppColors.divider),
        const SizedBox(height: 12),
        Text(AppLocalizations.of(context)!.clientTicketSeat, style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
        const SizedBox(height: 4),
        Text(place, style: TextStyle(fontSize: 32, fontWeight: FontWeight.w900, color: color)),
      ]),
    );
  }
