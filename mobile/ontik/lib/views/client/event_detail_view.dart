...
class EventDetailView extends ConsumerWidget {
  ...
  Widget _buildContent(BuildContext context, EventDetailState state) {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text(event.categorieNom ?? '', style: const TextStyle(color: Colors.grey)),
            ],
          ),
          const SizedBox(height: 16),
          ...
          const SizedBox(height: 16),
          if (state.availableSeats.isNotEmpty) ...[
            Text('Available Seats: ${state.availableSeats.where((s) => s.disponible).length}/${state.availableSeats.length}',
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            if (event.prixMin != null && event.prixMax != null)
              Text('Price: \$${event.prixMin} - \$${event.prixMax}', style: const TextStyle(fontSize: 16, color: Colors.green)),
            const SizedBox(height: 8),
          ],
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: event.idEvenement != null ? () => context.push('/reservation/${event.idEvenement}') : null,
